...
void ExtractTest::testExtraction_data()
{
    ...
    const auto setupRow = [](const char *name,
                             ...
                             QVector<Archive::Entry *> entriesToExtract,
    ...
    setupRow("extract the whole simplearchive.tar.gz", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 4);
    ...
    setupRow("extract the whole one_toplevel_folder.zip", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 9);
    ...
    setupRow("extract the whole one_toplevel_folder .7z", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 9);
    ...
    setupRow("zip with empty folders", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 5);

    archivePath = QFINDTESTDATA("data/empty_folders.tar.gz");
    setupRow("tar with empty folders", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 5);
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.bz2");
    setupRow("extract all entries from a bzip2 - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.xz");
    setupRow("extract all entries from a xz - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.lzma");
    setupRow("extract all entries from a lzma - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.Z");
    setupRow("extract all entries from a compress (.Z)-compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.tar.lz");
    setupRow("extract all entries from a lzipped tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);
    ...
    if (PluginManager().supportedMimeTypes().contains(QLatin1String("application/x-tzo"))) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.lzo");
        setupRow("extract all entries from a lzop - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);

        archivePath = QFINDTESTDATA("data/test.png.lzo");
        setupRow("extract the single - file test.png.lzo", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);
    }
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("lrzip")).isEmpty()) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.lrz");
        setupRow("extract all entries from a lrzip - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);

        archivePath = QFINDTESTDATA("data/test.txt.lrz");
        setupRow("extract the single - file test.txt.lrz", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);
    } else {
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("zstd")).isEmpty()) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.zst");
        setupRow("extract all entries from a zst - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);

        archivePath = QFINDTESTDATA("data/test.txt.zst");
        setupRow("extract the single - file test.txt.zst", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);
    } else {
    ...
    if (!QStandardPaths::findExecutable(QStringLiteral("lz4")).isEmpty()) {
        ...
        archivePath = QFINDTESTDATA("data/simplearchive.tar.lz4");
        setupRow("extract all entries from a lz4 - compressed tarball with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);

        archivePath = QFINDTESTDATA("data/test.txt.lz4");
        setupRow("extract the single - file test.txt.lz4", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);
    } else {
    ...
    archivePath = QFINDTESTDATA("data/simplearchive.xar");
    setupRow("extract all entries from a xar archive with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 6);

    archivePath = QFINDTESTDATA("data/hello-1.0-x86_64.AppImage");
    setupRow("extract all entries from an AppImage with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 7);

    archivePath = QFINDTESTDATA("data/hello-1.0-x86_64.AppImage");
    setupRow("extract selected entries from an AppImage with path",
             archivePath,
             QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("usr/bin/hello"), QString())},
             optionsPreservePaths,
             3);
    ...
    if (sevenZipFormat.supportsMultiVolume()) {
        ...
        setupRow("extract all entries from a multivolume 7z archive with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 3);
    } else {
    ...
    setupRow("extract all entries from a multivolume rar archive with path", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 3);
    ...
    setupRow("extract selected entries from rpm with path",
             archivePath,
             QVector<Archive::Entry *>{new Archive::Entry(this, QStringLiteral("boot/sunxi-spl.bin"), QString())},
             optionsPreservePaths,
             2);
    ...
    setupRow("extract the whole one_toplevel_folder.arj", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 9);
    ...
    setupRow("extract the single - file test.z", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.zz");
    setupRow("extract the single - file test.zz", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.txt.gz");
    setupRow("extract the single - file test.txt.gz", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.txt.bz2");
    setupRow("extract the single - file test.txt.bz2", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.png.lzma");
    setupRow("extract the single - file test.png.lzma", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.svgz");
    setupRow("extract the single - file test.svgz", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);

    archivePath = QFINDTESTDATA("data/test.sit");
    setupRow("extract the whole test.sit", archivePath, QVector<Archive::Entry *>(), optionsPreservePaths, 1);
